...
Slingshot.createDirective = function (name, service, options) {
  if (Slingshot._directives[name]) throw new Error('Directive \'' + name + '\' already exists');

  const restrictions = Slingshot.getRestrictions(name);
  setDefaults(options, restrictions);

  Slingshot._directives[name] = new Slingshot.Directive(service, options)
...
Slingshot.Directive = function (service, directive) {
  ...
  setDefaults(directive, service.directiveDefault);
...
Object.assign(Slingshot.Directive.prototype, {
  ...
  getInstructions: async function (method, file, meta) {
    const instructions = await this.storageService().upload(
      method,
      this._directive,
      file,
      meta,
  ...
  */
  ...
  requestAuthorization: async function (context, file, meta) {
    ...
    const restrictions = {
      authorize: this._directive.authorize,
      maxSize: this._directive.maxSize,
      allowedFileTypes: this._directive.allowedFileTypes,
    };
...
Meteor.methods({
  ...
  'slingshot/uploadRequest': async function (directiveName, file, meta) {
    ...
    const directive = Slingshot.getDirective(directiveName);
    ...
    return await directive.getInstructions(this, file, meta);
  },
});
